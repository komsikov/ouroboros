...
- `OUROBOROS_REVIEWS_DISABLED=true` disables every LLM review surface: advisory pre-review, commit triad, scope review, plan review, deep self-review, skill review, and hub install/update reviews. When it is unset, `OUROBOROS_FIXED_INFRA_MODELS=true` disables reviews by default; explicit `OUROBOROS_REVIEWS_DISABLED=false` overrides that fallback. Deterministic checks such as load errors, grants, dependency install, and staged-fingerprint revalidation remain active.
...
| OUROBOROS_REVIEWS_DISABLED | unset | Full review kill switch. `true` skips advisory, triad, scope, plan, deep self-review, skill review, and hub install/update review calls. If unset, `OUROBOROS_FIXED_INFRA_MODELS=true` disables reviews by default; explicit `false` keeps reviews enabled. |
