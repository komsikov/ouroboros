...
# Playwright Chromium/WebKit native system deps + browser binaries so browser
# tools work out of the box. Run as root, before the USER switch, because
# install-deps drives apt. PLAYWRIGHT_BROWSERS_PATH=0 stores the browsers
# alongside the package so the app user can read them.
