...
_RUNTIME_MODES = _rows(("value", "label", "tone", "className", "copy"), (
    ("light", "Лёгкий", "Безопаснее", "light", "Самомодификация основного репозитория отключена. Лучший вариант для знакомства с Ouroboros без самомодификации."),
    ("advanced", "Стандартный", "По умолчанию", "advanced", "Самомодификация эволюционного слоя разрешена (текущее поведение). Защищённые файлы ядра/контрактов/релизов охраняются в стандартном режиме."),
    ("pro", "Про", "Расширенный", "pro", "Прямой режим защищённых поверхностей. Редактирование защищённых файлов ядра/контрактов/релизов разрешено, но коммиты по-прежнему проходят через триаду и проверку области."),
))
