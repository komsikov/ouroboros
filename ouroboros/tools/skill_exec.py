    ...
    **_kwargs: Any,
    ...
    with skill_span(
        ...
        try:
            ...
            record_skill_result(_otel_skill_span, output=f"payload unreadable before spawn: {exc}", is_error=True)
            return (
                f"⚠️ SKILL_EXEC_ERROR: skill {skill_name!r} payload became unreadable "
                f"right before execution ({exc})."
            )
        if spawn_hash != current_hash:
            record_skill_result(_otel_skill_span, output="payload changed between review and spawn", is_error=True)
            return (
